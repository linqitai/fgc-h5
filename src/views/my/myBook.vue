						...
						<div class="list">
							<div class="item" v-for="item in list4" :key="item.id">
								<div class="flex">
									...
									<div class="line margT6"><i :class="item.type==12?'mainAdornColor':''">{{item.type | platformBookType}}</i><i class="mainAdornColor" v-if="item.type==7">给{{item.toUserName}}</i>后拥有感恩券 {{item.currentTicketNum}}</div>
								</div>
